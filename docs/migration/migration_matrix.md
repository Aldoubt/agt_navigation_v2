...
| `agt_system_manager` | 1/2/4/6/7 | 统一建图会话、离线静态候选门禁、Teach Mapping MVP 离线和安全状态来源修复完成 | `/agt/mapping/manage_session` 原子持久化 capture/static-build/candidate/register 状态；START 独占路径并强制 mapping bag；FINALIZE 先保存在线预览、正常停止并校验 PCD/hash/bag，再调用 `agt_map_processing` 生成射线 + `ground_temporal` 候选；位姿、地面拟合、裁剪、报告计数、画布边缘任一失败都禁止 Qt，离线阶段可重试；健康节点从 `/agt/safety/status` 获取急停锁存与 `navigation_ready`，不再把缺失 optional emergency_stop 输入当成 active | 用真实 MID360 验证 SaveMap/退出落盘、离线失败重试、管理器重启恢复、磁盘不足和 Ctrl+C 重复信号；在 Qt 完成当前新候选的人工障碍审查和 planner-only 长路径预览，再做实车验收 |
