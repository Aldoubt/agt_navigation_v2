...
| --- | --- | --- | --- | --- |
| `agt_interfaces` | 1/8 | TASK-13/14、waypoint/mapping/Mission 与统一状态接口完成 | coverage、waypoint、mapping、Mission Action，以及 RobotState/Mission/Map/Bag msg/srv 通过 ROSIDL C++/Python 生成；核心状态使用枚举与稳定错误码 | 用真实建图和 Mission launch 验证 Action 超时、取消和进程异常分支；后续字段变更需兼容性评审 |
...
| `agt_navigation` | 6/8 | 项目多点 Action、版本化离线任务组、planner-only 预览和统一定位 freshness/task gate 完成；完整实车 lifecycle 验收待完成 | schema-v1 任务组提供原子保存/索引/备份、地图版本与 YAML/image/PCD hash 绑定、旧 points/theta 导入及任务点端点栅格校验；相邻点是 Nav2 有序目标而非直线路径，Task Library 用 `ComputePathToPose` 预览实际绕行；预览逐段发布 current/total 状态并以显式正有限超时 fail-closed 清空 `/plan`；navigation/offline Qt 只保留 Task Library，READY 底图编辑/保存和旧拓扑任务入口关闭，新建后直接两点击定姿、保存后退出布点；`ExecuteWaypointTask` 兼容新旧 JSON，只向 Nav2 `FollowWaypoints` 派发并在接收及执行中检查 10 s 定位 freshness、2 s TaskReadiness 和安全状态；offline/mapping profile 禁止执行 | 用正式 READY 地图版本完成 Qt 关闭重载、内容/几何失配和 Action 取消集成验收；实车验证多点成功、失败、missed waypoint 和急停；补 Nav2 lifecycle 与统一运行记录，再实现最小 mission orchestrator |
...
| `agt_system_manager` | 1/2/4/6/7 | 统一建图会话、RobotState 聚合、Teach Mapping 与安全状态来源修复完成 | 保留受管建图链；新增 2 Hz + 输入变化即时 `/agt/system/robot_state` 和一次性 service，只消费权威 topic，地图身份来自 map manager，Nav2/safety/chassis/localization 等过期证据保持 UNKNOWN/blocker；三包 build、19 个 colcon test 目标和六节点 launch smoke 通过 | 用真实 MID360 验证建图落盘；联调 map/bag facade 后验证 RobotState active-map/Bag freshness；完成 Qt/Web 共同状态显示和实车验收 |
