...
| `agt_sensor_adapters` | 3 | baseline 完成 | Livox 驱动已迁入，MID360 原始 CustomMsg 输入、统一 topic 和短 bag 回放通过；Web 传感器 profile 可选择白名单网络 JSON 并等待健康结果 | 实机验证网络、QoS、频率、时间戳、丢包和长时间运行稳定性 |
