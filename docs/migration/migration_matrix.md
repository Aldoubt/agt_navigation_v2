...
| `agt_experiment_manager` | 7 | ROS 业务 facade 离线完成 | 新增 list/manage/status typed API，唯一持有 record/playback 子进程；显式 profile、固定根路径、配置/Git 快照、异常进程 ERROR、重启 RUNNING->INTERRUPTED 和 summary/report 通过；建图自动使用 mapping profile | 接入完整健康/导航事件，用真实 rosbag 长包验证正常停止、异常中断、空间统计和统一报告 |
