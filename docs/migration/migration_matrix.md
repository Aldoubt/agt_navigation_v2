...
| `agt_navigation` | 6/8 | TASK-07 与项目多点桥接离线完成 | 新增 `ExecuteWaypointTask` 到 Nav2 `FollowWaypoints` 状态机；拒绝重复追加、异常坐标、地图越界、无限循环和未就绪安全状态，取消会传递到子 Action；无车模拟 0.5/1.0 m 两点执行返回成功且 missed 为空，整套节点干净退出 | 使用实车 Qt 保存的任务 JSON 验证成功、失败、取消、急停和有限循环；原生 Qt Start 按钮仍不作为可靠入口 |
...
| `agt_ui_bridge` | 6/8 | Bunker 双 profile baseline；Qt 启动加固 | 上游 Qt5 源码不修改；导航 YAML 在启动前校验并与 Nav2 同源，错误持久化路径会清除；启动器清理 Snap GUI/loader 环境并报告越界拓扑点 | 实机验证同步地图首次显示；vendor 手动文件选择仍需操作规程约束，未来前端直接调用项目多点 Action |
