...
| `agt_navigation` | 6/8 | 项目多点 Action 与 planner-only 离线预览完成；任务编排边界已预留 | `ExecuteWaypointTask` 到 Nav2 `FollowWaypoints` 状态机；Qt 执行只消费 Action且 ROS 派发不占 GUI 线程；独立 offline profile 以显式任务点逐段调用 `ComputePathToPose` 并只发布 `/plan`，不启动运动链；拒绝异常/越界/无限循环/未就绪安全状态并传播取消 | 实车验证多点成功、失败、取消和急停；用昨日大地图验证多段离线预览；先增加统一运行记录，再实现最小 mission orchestrator |
