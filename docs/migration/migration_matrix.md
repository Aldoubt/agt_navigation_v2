...
| `agt_map_processing` | 5 | 大包混合静态障碍补全、完整车体扫掠和三组可通行性对照已离线回放 | 点云时间同步与 canonical 多边形裁剪；全部 8,072 个里程计位姿形成 72,469 个扫掠栅格；RANSAC/时序/高度对照消费全部 8,073 帧且地面拟合零失败，三图均通过同一长距离规划；分层阈值仍显式不可执行 | 优先人工核验保守 `ground_temporal` 与 v5；实测整车含 MID360 最高点后才能评审高度分层可执行性，再增加动态真值和独立 bag 验证 |
...
| `agt_navigation` | 6/8 | 项目多点 Action 与 planner-only 离线预览完成；任务编排边界已预留 | `ExecuteWaypointTask` 到 Nav2 `FollowWaypoints` 状态机；Qt 执行只消费 Action且 ROS 派发不占 GUI 线程；独立 offline profile 以显式任务点逐段调用 `ComputePathToPose` 并只发布 `/plan`，不启动运动链；RViz 分层显示静态图、膨胀 costmap 与 canonical 多边形车体；拒绝异常/越界/无限循环/未就绪安全状态并传播取消 | 实车验证多点成功、失败、取消和急停；用新障碍图验证多段离线预览与 footprint 净距；先增加统一运行记录，再实现最小 mission orchestrator |
