...
V25-08 的 Navigation Capability、MAP/ROUTE/LOCAL、地图产品、Route/Runtime Path 以及
state-estimation ownership 语义以
[`navigation_semantics.md`](../architecture/navigation_semantics.md) 为准。当前导航启动顺序、
Qt5/Nav2/安全数据流以及门禁矩阵见
...
- `/agt/map/global_occupancy`: 导航模式下由 Nav2 map server 发布的已保存静态地图，`map` frame。
- `/agt/map/local_occupancy`: V25-08 预留的 canonical rolling local-environment grid，`odom` frame；
  当前没有正式 publisher，不得把现有 Nav2 internal local costmap 冒充为该 topic。未来实现必须保持
  transient/rolling 语义，不写回版本化 global map。
...
`agt_safety` 或命令 guard；当前 MAP runtime 的真实导航速度链是
`/agt/navigation/cmd_vel_raw -> /agt/navigation/cmd_vel -> /agt/safety/cmd_vel -> /agt/chassis/cmd_vel`。
未来 ROUTE/LOCAL backend 也必须进入同一 project safety/chassis boundary，不得新增绕过
`agt_safety` 的最终底盘命令路径。CAN 接口的 up/down 配置不属于 ROS/Web 接口，必须由主机管理员预先完成。
...
`/agt/localization/status_text` 参与控制。V25-08 只冻结未来 mode-aware readiness 名称，
没有修改当前 `EvaluateTaskReadiness.srv`。
...
- `/agt/coverage/execute`: `agt_interfaces/action/ExecuteCoverageTask`，串联语义加载、规划、验证、可选修复和当前 Nav2 `FollowPath` backend。
...
当前 MAP-oriented runtime 存在以下 Nav2 native/兼容接口：

- `/navigate_to_pose`: `nav2_msgs/action/NavigateToPose`。
- `/navigate_through_poses`: `nav2_msgs/action/NavigateThroughPoses`。
- `/follow_waypoints`: `nav2_msgs/action/FollowWaypoints`，由当前项目任务实现内部使用。
- `/goal_pose`: Qt5/RViz2 的 `geometry_msgs/PoseStamped` 兼容入口。

正式 project navigation capability 是：
...
`ExecuteWaypointTask` 当前 MAP backend 内部使用 Nav2，但 Mission、BT、Qt/Web 不得把
`/navigate_to_pose`、`/navigate_through_poses`、`/follow_waypoints` 或 `FollowPath` 当作新的业务边界。
`/goal_pose -> agt_goal_pose_bridge -> NavigateToPose` 仍是兼容/调试路径，不订阅 TaskReadiness、
active-map identity 或 chassis-connected 共享门禁；正式版本绑定任务必须使用
`/agt/navigation/execute_waypoint_task`。

当前 MAP runtime 的速度链是：

```text
Nav2 controller
  -> /agt/navigation/cmd_vel_raw
  -> collision monitor / project safety boundary
  -> /agt/navigation/cmd_vel
  -> /agt/safety/cmd_vel
  -> /agt/chassis/cmd_vel
```

未来 ROUTE/LOCAL 可以替换或复用内部 planner/path follower/controller，但必须保持 project
capability、cancel、readiness、安全和 chassis ownership；不得让 Mission/BT/前端直接发布最终速度。

## TF 责任

- `odom -> base_footprint`: `agt_mapping_fast_livo2_adapter` 唯一发布。
- authoritative `map -> odom`: localization subsystem 唯一拥有；任一运行时 profile 中只能有一个
  selected publisher。
- 当前 baseline 中 `agt_localization` package 的 `agt_relocalization` node 在
  `publish_tf=true` 时唯一发布 `map -> odom`。
- 未来若 `agt_localization_fusion` 成为 selected publisher，必须先关闭 baseline publisher 的
  `publish_tf`；GTSAM/GNSS/loop/relocalization backend 不得另开第二个 TF publisher。
