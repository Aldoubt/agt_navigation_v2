...
2026-07-29 针对性修复：直接启动 `agt_bringup/system.launch.py` 时同步启动
`agt_map_manager`、`agt_robot_state_aggregator` 和 `agt_mission_manager`，保证 Qt5
navigation profile、RobotState 和 TaskReadiness 都能消费同一 `/agt/maps/active` READY
活动地图上下文；外层 system manager profile 仍可用 `start_system_health:=false` 避免重复。

2026-07-29 P0 执行链修复：新增机器人端 Task Registry 服务与 Navigation Session 状态，
`ExecuteWaypointTask` 正式入口改为 map/task ID + revision + content hash + `client_request_id`。
Qt5 Task Library 不再发送本机绝对 `task_file`，断联后以 `/agt/navigation/session_status` 恢复状态；
legacy `task_file` 仅作为默认关闭的同机 CLI/debug 入口保留。
...
| `agt_bringup` | 1/3/4/5/6/8 | Bunker Qt5 FAST-LIO baseline、一次性自动重定位、定位 freshness gate 和 Teach Mapping 复扫组合已接线 | mapping/navigation 共用总入口；直接总控同时启动 map manager、health/readiness、RobotState 和 Mission 后端，确保 `/agt/maps/active` 驱动 Qt/TaskReadiness 的 READY 地图身份；mapping 默认 RViz；navigation 将同一 map YAML 注入 Nav2 与任务型 Qt；定位 5 s 验证周期配套 10 s freshness gate；lifecycle gate 在暂停后 RESUME、启动拒绝后 RESET 重试且 fail-closed；Teach Mapping 只用绑定的 Bootstrap Map 启动单套 navigation，默认断车/禁执行/录包、固定不自动 start、限速 0.10 m/s | 用实车验证完整建图关机、复扫取消/急停/录包、候选 YAML 自动重定位、同源 PCD 重定位和项目多点 Action 闭环；确认 Bunker vendor 退出告警不影响停车 |
...
| `agt_navigation` | 6/8 | 项目多点 Action、机器人端任务仓库、可靠会话、版本化离线任务组、planner-only 预览和统一定位 freshness/task gate 完成；完整实车 lifecycle 验收待完成 | schema-v1 任务组提供原子保存/索引/备份、地图版本与 YAML/image/PCD hash 绑定、旧 points/theta 导入及任务点端点栅格校验；Task Registry 以 map/task ID 解析任务并验证 revision/content hash；`ExecuteWaypointTask` 正式入口不接受前端文件路径，使用 `client_request_id` 幂等并发布 reliable/transient-local `NavigationSessionStatus`；相邻点是 Nav2 有序目标而非直线路径，Task Library 用 `ComputePathToPose` 预览实际绕行；预览逐段发布 current/total 状态并以显式正有限超时 fail-closed 清空 `/plan`，后续段从前一段 Nav2 实际终点继续，避免目标容差导致断线或虚假的 lethal-start 失败；navigation/offline Qt 只保留 Task Library，READY 底图编辑/保存和旧拓扑任务入口关闭，新建后直接两点击定姿、保存后退出布点；`ExecuteWaypointTask` 只向 Nav2 `FollowWaypoints` 派发并在接收及执行中检查 10 s 定位 freshness、2 s TaskReadiness 和安全状态；offline/mapping profile 禁止执行 | 用正式 READY 地图版本完成 Qt 关闭重载、内容/几何失配和 Action 取消集成验收；实车验证多点成功、失败、missed waypoint、重复 request 和急停；补 Nav2 lifecycle 与统一运行记录，再实现最小 mission orchestrator |
