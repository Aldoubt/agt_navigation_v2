...
| ROUTE navigation core | SYSTEM-INTEGRATED | V25-09B loads immutable READY Route Assets, resolves exact TaskGroup->Route bindings, freezes active map-frame segments into odom RuntimePaths and tracks them through Nav2 `FollowPath` with zero Global Planner requests. MAP/ROUTE Action tests are green; explicit parent-cancel, Safety, Localization and TaskReadiness child-cancel tests are now registered. A controller-only real Nav2 smoke (`FollowPath -> collision_monitor -> safety -> simulator`) is implemented; its local execution result and the later full formal Route->controller chain remain the next software gates. |
