...
| Offline asset workspace / compliance tooling | IMPLEMENTED | `agt_offline_assets` provides immutable Dataset/Calibration/Recipe lineage, READY->DRAFT map cloning with parent hash, `map_edit_record.yaml`, `init-map`, `clone-map-revision`, `validate-map`, MappingSession evidence ingestion and one-way READY promotion. |
...
| Semantic Route MVP / preview / feasibility | IMPLEMENTED | `semantic_boustrophedon` derives annotated-row candidates through a registered `straight` ConnectorPlannerBackend; existing full-footprint validator + semantic field/exclusion/keepout gate produce feasibility and preview. Unknown connector backends fail closed. |
| Reverse-aware / kinematic connector planner | IMPLEMENTED | V25-09D adds a self-contained canonical Reeds-Shepp backend with signed L/R/S primitives, deterministic symmetry candidates, endpoint/spacing bounds, forward-only fail-closed behavior and F/R cusp RouteSegment splitting. Hybrid-A* remains RESERVED. |
| Obstacle-aware local connector planner | IMPLEMENTED | V25-09E adds a bounded offline Hybrid A* backend with grid/origin transforms, occupancy/unknown/semantic checks, forward/reverse motion primitives, deterministic search and collision-checked Reeds-Shepp analytic expansion. It is not a Nav2 global planner; dynamic/local-map planning remains deferred. |
...
| ROUTE navigation core | SYSTEM-INTEGRATED | V25-09B software acceptance is complete: READY Route resolution, MAP/ROUTE Action selection, odom-segment projection, real Nav2 `FollowPath`, collision monitor, safety chain, simulator, parent cancel/runtime-gate cancellation, and the full formal synthetic Route->ExecuteWaypointTask->controller chain all passed with zero Global Planner requests. Vehicle acceptance remains separate. |
| Sparse correction / anchor recovery | IMPLEMENTED | V25-10 code layer now includes pure correction math/policy, state-specific TRACKING/RECOVERING/LOST gates, monotonic correction generation, `global_correction_manager` as production `map -> odom` authority, automatic recovery trigger policy/manager, and reuse of existing configured candidate seeds as anchors. Local build/unit/system smoke is the next gate; bag and vehicle acceptance remain pending. |
...
| Qt/Web operator tooling | IMPLEMENTED | Existing Qt5 editor now exposes Map Revision clone/state/hash, DRAFT-only editing, map validation, unified layers and offline Route generate/validate controls. It remains read-only with respect to READY assets and owns no runtime state. |
