...
| Offline asset workspace / compliance tooling | IMPLEMENTED | `agt_offline_assets` provides immutable Dataset/Calibration/Recipe lineage, independent Capture Rig validation, `init-map`, `validate-map`, MappingSession evidence ingestion and one-way READY promotion. |
...
| Qt/Web operator tooling | IMPLEMENTED | Existing Qt5 alignment/PCD/semantic viewer is the reusable offline review surface; unified route-studio shell and route editing integration remain the next UI increment. It remains read-only with respect to READY assets and owns no runtime state. |
