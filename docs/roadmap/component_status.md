...
| Offline asset workspace / compliance tooling | IMPLEMENTED | `agt_offline_assets` provides immutable Dataset/Calibration/Recipe lineage, `init-map`, `validate-map`, MappingSession evidence ingestion and one-way READY promotion. Core package/contract tests passed locally before the replay increment; the new replay/ingestion tests are the current gate. |
| Offline bag replay -> PROCESSING workspace | IMPLEMENTED | `agt_system_manager/replay_mapping_to_workspace.py` preflights bag/profile hashes, reuses managed MappingSession for mapping/finalization, records a derived mapping bag and freezes session-frame candidate/PCD evidence under `processing/mapping_session/`. Canonical alignment/materialization remains the next gate. |
...
| Canonical alignment / materialization tooling | RESERVED | Next V25-09A increment: apply `mapping_session -> map`, produce canonical PCD/navigation assets, alignment report and map-quality evidence before READY. |
