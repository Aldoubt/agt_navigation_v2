...
ROUTE_DEBUG_LAYER_KEYS = (
    "base.navigation", "base.no_go", "structure.turn_zones", "structure.aisles",
    "structure.vehicle_safe_lane", "coverage.order", "coverage.requests",
...
class RouteDebugSceneController(QObject):
    ...
    def _z_for_layer(layer_key: str) -> float:
        order = {
            "base.navigation": -80.0, "diagnostics.raw": -70.0,
            "diagnostics.geometry": -69.0, "diagnostics.padding": -68.0,
            "diagnostics.unknown": -67.0, "base.no_go": -20.0,
            "structure.turn_zones": -10.0, "structure.aisles": 0.0,
            "structure.vehicle_safe_lane": 5.0, "coverage.order": 10.0,
            "coverage.requests": 12.0, "motion.forward_candidates": 20.0,
            "motion.forward_selected": 25.0, "motion.reverse": 27.0,
            "diagnostics.conflicts": 40.0, "diagnostics.failed": 45.0,
        }
    ...
    def set_failure_focus(self, enabled: bool) -> None:
        ...
        for item in self._feature_items.values():
            payload = item.data(1)
            is_failure = bool(payload.get("is_failure", False)) if isinstance(payload, Mapping) else False
            item.setOpacity(1.0 if (not self._failure_focus or is_failure) else 0.12)
    ...
    def _refresh_visibility(self) -> None:
        for key, group in self._groups.items():
            group.setVisible(self._active and self._layer_visible.get(key, True))

    def _on_scene_selection_changed(self) -> None:
        selected = [item for item in self._scene.selectedItems()
                    if isinstance(item.data(0), str) and item.data(0) in self._feature_items]
        if selected:
    ...
    def _draw_selection_footprint(self, payload: Mapping[str, Any]) -> None:
        ...
        if not isinstance(polygon, list) or len(polygon) < 3:
            return
        item = QGraphicsPolygonItem(QPolygonF([_scene_xy(p[0], p[1]) for p in polygon]))
    ...
    def _rgba_pixmap(rgba: np.ndarray) -> QPixmap:
        ...
        h, w, _ = array.shape
        image = QImage(array.data, w, h, 4 * w, QImage.Format_RGBA8888).copy()
        return QPixmap.fromImage(image)

    def _add_raster(self, layer_key: str, rgba: np.ndarray, dataset: RouteDebugDataset) -> None:
        navigation = dataset.navigation
        if navigation is None:
            return
        item = QGraphicsPixmapItem(self._rgba_pixmap(np.flipud(rgba)))
        maximum_y = navigation.origin_y_m + navigation.height * navigation.resolution_m
    ...
    def _render_navigation(self, dataset: RouteDebugDataset) -> None:
        ...
        rgba[occupancy == FREE] = np.array([241, 245, 249, 190], dtype=np.uint8)
        rgba[occupancy == UNKNOWN] = np.array([100, 116, 139, 170], dtype=np.uint8)
        rgba[occupancy == OCCUPIED] = np.array([15, 23, 42, 230], dtype=np.uint8)
        self._add_raster("base.navigation", rgba, dataset)
    ...
    def _render_source_masks(self, dataset: RouteDebugDataset) -> None:
        ...
        specs = (
            ...
            ("diagnostics.unknown", navigation.occupancy == UNKNOWN, (59, 130, 246, 105)),
        )
        for layer_key, mask, color in specs:
            rgba = np.zeros((navigation.height, navigation.width, 4), dtype=np.uint8)
            rgba[np.asarray(mask, dtype=bool)] = np.array(color, dtype=np.uint8)
            self._add_raster(layer_key, rgba, dataset)
    ...
    def _render_feature(self, feature: Mapping[str, Any]) -> None:
        ...
        if geometry_type == "LineString":
            item = self._line_item(geometry.get("coordinates") or [], layer_key)
            ...
            if len(coords) >= 2:
                item = self._point_item(float(coords[0]), float(coords[1]), layer_key,
                                        str(props.get("feature_kind", "")))
        if item is None:
            return
        payload = dict(props.get("inspector") or {})
        payload.update({
            "feature_id": feature_id, "feature_kind": props.get("feature_kind"),
            "status": props.get("status"), "source_asset": props.get("source_asset"),
            "source_id": props.get("source_id"), "source_field": props.get("source_field"),
    ...
    def _line_item(coordinates, layer_key: str):
        ...
        color, width, _ = _LAYER_STYLE.get(layer_key, ("#94a3b8", 1.5, 0))
    ...
    def _polygon_item(coordinates, layer_key: str):
        if not isinstance(coordinates, list) or len(coordinates) < 3:
            return None
        color, width, alpha = _LAYER_STYLE.get(layer_key, ("#94a3b8", 1.5, 35))
        item = QGraphicsPolygonItem(QPolygonF([_scene_xy(xy[0], xy[1]) for xy in coordinates]))
        item.setPen(QPen(_qcolor(color, 230), float(width) * 0.02))
        item.setBrush(QBrush(_qcolor(color, alpha)))
        return item

    @staticmethod
    def _point_item(x: float, y: float, layer_key: str, feature_kind: str):
        color, _, alpha = _LAYER_STYLE.get(layer_key, ("#ef4444", 2.0, 100))
    ...
    def _add_sequence_label(self, source_item: QGraphicsItem, text: str, layer_key: str) -> None:
