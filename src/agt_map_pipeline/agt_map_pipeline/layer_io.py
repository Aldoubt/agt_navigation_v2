...
def write_prepare_layers(project_dir, navigation, structure=None, corridor=None, aisle_graph=None, *, evidence_only=False):
    ...
    scalar_layers = {
        "terrain/ground_height.npy": navigation.ground_height_m,
        "terrain/ground_valid.npy": navigation.ground_valid,
        "terrain/ground_confidence.npy": getattr(structure, "ground_confidence", navigation.ground_valid.astype(float)),
        "terrain/slope_deg.npy": navigation.slope_deg,
        "terrain/step_m.npy": navigation.step_m,
        "obstacle/obstacle_count.npy": navigation.obstacle_count,
        "obstacle/obstacle_mask.npy": navigation.obstacle_count >= navigation.config.minimum_obstacle_points,
    }
    for key, value in scalar_layers.items():
        rec = _save(root, key, value)
        out[key.rsplit("/", 1)[0] + "." + Path(key).stem] = {"path": str(Path("layers") / key), "sha256": rec["sha256"]}

    occupancy_key = "evidence/navigation_occupancy.npy" if evidence_only else "navigation/occupancy.npy"
    occupancy_id = "evidence.navigation_occupancy" if evidence_only else "navigation.occupancy"
    rec = _save(root, occupancy_key, navigation.occupancy)
    out[occupancy_id] = {"path": str(Path("layers") / occupancy_key), "sha256": rec["sha256"]}
