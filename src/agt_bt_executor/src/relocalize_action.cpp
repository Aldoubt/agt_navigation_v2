...
BT::PortsList Relocalize::providedPorts() {
  ...
  ports.insert(BT::InputPort<bool>("use_last_valid_pose", true, "use last valid pose")); ports.insert(BT::InputPort<bool>("use_configured_candidates", true, "use configured candidates")); ports.insert(BT::InputPort<bool>("publish_debug", false, "publish diagnostic output")); return ports;
}
...
bool Relocalize::resultSuccess(const Result & result) const { RCLCPP_INFO(node_->get_logger(), "Relocalize result success=%s error=%u reason=%s fitness=%.3f", result.success ? "true" : "false", result.error_code, result.failure_reason.c_str(), result.final_status.fitness_score); return result.success; }
