...
class ExperimentBusinessFacade:
    ...
    def create_experiment(self, values: Mapping[str, str]) -> dict[str, Any]:
        ...
        experiment_id = self.manager.create(
            ...
            launch_profile=str(values.get("nav2_profile", "")),
            launch_arguments={
    ...
    def manage(self, operation: int, values: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if operation == 4:
            ...
            return self.status()
        if operation == 5:
            self.create_experiment({key: str(value) for key, value in values.items()})
            return self.status()
        if operation == 6:
            if not experiment_id:
                raise ExperimentError("experiment_id is required")
            self.manager.finalize(experiment_id)
            return self.status()
        if operation == 7:
            if not experiment_id:
                raise ExperimentError("experiment_id is required")
            self.manager.interrupt(experiment_id)
            return self.status()
