...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr &node)
{
  ...
  try_declare.template operator()<bool>("common.publish_tf", true);
...
void LIVMapper::initializeFiles() 
{
  ...
  if (pcd_save_interval > 0) {
    ...
    fout_pcd_pos.open(pcd_output_directory + "/scans_pos.json", std::ios::out);
  }
...
void LIVMapper::initializeSubscribersAndPublishers(rclcpp::Node::SharedPtr &node, image_transport::ImageTransport &it_)
{
  ...
  pubLaserCloudLidar = this->node->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_registered_lidar", 100);
...
{    
  ...
  *pcl_w_wait_pub = *laserCloudWorld;
  ...
  publish_frame_lidar(pubLaserCloudLidar);
...
void LIVMapper::savePCD()
{
  ...
  if (pcl_wait_save->points.size() > 0 || pcl_wait_save_intensity->points.size() > 0)
  {
    ...
    std::string downsampled_points_dir = pcd_output_directory + "/all_downsampled_points.pcd";
...
void LIVMapper::publish_frame_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
  ...
  if (pcd_save_en)
  {
    ...
    if ((pcl_wait_save->size() > 0 || pcl_wait_save_intensity->size() > 0) && pcd_save_interval > 0 && scan_wait_num >= pcd_save_interval)
    {
      std::filesystem::create_directories(pcd_output_directory);
      pcd_index++;
      string all_points_dir(pcd_output_directory + "/" + to_string(pcd_index) + ".pcd");
      pcl::PCDWriter pcd_writer;
      if (pcd_save_en)
      {
...
{
  PointCloudXYZI::Ptr cloud(dense_map_en ? feats_undistort : feats_down_body);
  if (cloud == nullptr || cloud->empty()) return;

  sensor_msgs::msg::PointCloud2 message;
  pcl::toROSMsg(*cloud, message);
  message.header.stamp = odomAftMapped.header.stamp;
...
void LIVMapper::publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr &pubOdomAftMapped)
{
  ...
  this->node->get_parameter("common.publish_tf", publish_tf);
  if (publish_tf) {
